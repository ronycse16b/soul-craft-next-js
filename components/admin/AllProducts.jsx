...
const AllProducts = () => {
  ...
  // track expanded rows by product id or index
  const [expanded, setExpanded] = useState(new Set());
  // map of refs for each row content
  ...
  // make sure the refs object has a ref for each product
  const ensureRef = (id) => {
    if (!contentRefs.current[id]) contentRefs.current[id] = { ref: null };
            ...
            <table className="min-w-[1000px] w-full text-xs text-gray-700 whitespace-nowrap">
              ...
              <tbody className="divide-y divide-gray-200">
                {products.map((item, index) => {
                  const id = item._id ?? index; // each product unique key
                  const isOpen = expanded.has(id); // ✅ define isOpen correctly
                  ...
                  return (
                    <Fragment key={id}>
                      ...
                      <tr className="hover:bg-gray-50 transition">
                        ...
                        <td className="px-3 py-1 border text-center">
                          {item.type === "variant"
                            ? item.variants?.reduce((a, v) => a + v.quantity, 0)
                            : item.quantity}
                        </td>
                        <td className="px-3 py-1 border text-center">
                      ...
                      {/* ==== Variant Table (with smooth transition) ==== */}
                      {item.type === "variant" && item.variants?.length > 0 && (
                        <tr>
